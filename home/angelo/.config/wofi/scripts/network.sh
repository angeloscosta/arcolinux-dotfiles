...
LIST=$(nmcli -w 3 --fields "$FIELDS" device wifi list | sed '/^--/d')
# For some reason wofi always approximartes character width 2 short... hmmm
...
CHENTRY=$(echo -e "$TOGGLE\nmanual\n$LIST" | uniq -u | $command --prompt "Wi-Fi SSID: ")
#echo "$CHENTRY"
...
if [ "$CHENTRY" = "manual" ] ; then
	...
	MSSID=$(echo "enter the SSID of the network (SSID,password)" | $command --prompt "Manual Entry: ")
	...
	if [[ $(echo "$KNOWNCON" | grep "$CHSSID") = "$CHSSID" ]]; then
		...
		if [[ "$CHENTRY" =~ "WPA2" ]] || [[ "$CHENTRY" =~ "WEP" ]]; then
			WIFIPASS=$(echo "If connection is stored, hit enter." | $answer_command --prompt "password")
		fi
